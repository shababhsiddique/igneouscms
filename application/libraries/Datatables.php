 ...
 *             Shabab Haider Siddique <shabab.h.siddique@gmail.com>
...
class Datatables {
                ...
                foreach ($miSearch as $val) {
                    if (preg_match("/(<=|>=|=|<|>)(\s*)(.+)/i", trim($val), $matches))
                        ...
                        if ($val == "~") {
                            ...
                            $range = explode('~', $val);
                            if ($range[0] != "")
                                $this->ci->db->where($this->select[$mColArray[$i]] . ' >=', $range[0]);
                            if ($range[1] != "")
                                $this->ci->db->where($this->select[$mColArray[$i]] . ' <=', $range[1]);
                        }
                    }
                    else
    ...
    protected function produce_output($charset) {
        ...
        foreach ($rResult->result_array() as $row_key => $row_val) {
            ...
            /**
             * Trim Leading/Trailing spaces and reduce large data to 100 Chars 
             */
            foreach($aaData[$row_key] as $someKey => $something){
                $aaData[$row_key][$someKey] = trim(strip_tags(trim($something)));
                if(strlen($aaData[$row_key][$someKey])>100){
                    $aaData[$row_key][$someKey] = substr($aaData[$row_key][$someKey], 0,95)."...";
                }
            }
