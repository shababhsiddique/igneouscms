/************************************************************************************************************
Custom Drag and drop folder tree
Copyright (C) 2013 Shabab Haider Siddique 
inspire from DTHMLGoodies.com, Alf Magne Kalleland

Shabab Haider Siddique
shababhsiddique@diu.edu.bd
shababhsiddique.wordpress.org
...
JSDragDropTree.prototype = {
    ...
    ,
    editItem : function(obj1,obj2)
    {                    
        var id = obj2.id.substring(8);
        ...
        popupWindow = window.open( url.join("/") ,'popUpWindow','height=700,width=800,left=10,top=10,resizable=yes,scrollbars=yes,toolbar=yes,menubar=no,location=no,directories=no,status=yes');
    }
    ,
    ...
    initTree : function()
    {
        ...
        if(this.deleteAllowed || this.renameAllowed){
            try{
                ...
                menuModel.addItem(0, "Edit", "","",false,"JSTreeObj.editItem");
        ...
        for(var no=0;no<menuItems.length;no++){
            ...
            var crrentInd = menuItems[no].id.replace(/[^0-9]/gi,'');
            ...
            var pub = aTag.getAttribute("data-published");
            chkb.type = "checkbox";            
            chkb.setAttribute("class","css-checkbox");            
            chkb.id = "cb_"+crrentInd;            
            chkb.setAttribute("data-id",crrentInd);
            chkb.setAttribute("data-published",pub);
            if(pub == 'published'){
                chkb.setAttribute("checked", "checked");
                chkb.setAttribute("onclick", "publishNode(this)");
            }else{                
                chkb.setAttribute("onclick", "publishNode(this)");
            }
            
            
            var chkblbl = document.createElement('label');
            chkblbl.setAttribute("class","css-label");            
            chkblbl.setAttribute("name","cb_"+crrentInd);            
            chkblbl.id = "cb_"+crrentInd;            
            chkblbl.setAttribute("for","cb_"+crrentInd);            
            
            menuItems[no].insertBefore(chkb, aTag);
            menuItems[no].insertBefore(chkblbl, aTag);
            /*Added*/
